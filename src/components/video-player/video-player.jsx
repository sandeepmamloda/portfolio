...
const VideoPlayerContent = function () {
  ...
  const innerRef      = useRef(null);
  const titleTimerRef = useRef(null);
  const cursorTimerRef = useRef(null); // ── NEW
  ...
  // ── NEW: Fullscreen cursor show/hide on mousemove ─────────────
  useEffect(() => {
    if (!isFullscreen) {
      // Fullscreen se bahar — cursor normal karo, timer clear karo
    ...
    const showCursor = () => {
      ...
      clearTimeout(cursorTimerRef.current);
      // 3 second idle ke baad cursor hide
      cursorTimerRef.current = setTimeout(() => {
        document.documentElement.style.cursor = "none";
      }, 3000);
    };

    // Mount pe bhi ek baar show karo
    showCursor();
  ...
  useEffect(() => {
    ...
    const onFullscreenChange = () => {
      ...
      // ── NEW: fullscreen pe class toggle — CSS cursor fix ──
      entering
