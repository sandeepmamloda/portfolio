...
const Moodboard = () => {
  ...
  // ── Header + card entrance animations ──────────────────────────────────────
  useLayoutEffect(() => {
    ...
    const timer = setTimeout(() => {
      const ctx = gsap.context(() => {
        ...
        // ── H1 — letter clip reveal (same as Journal) ──
        ...
        // ── Desktop cards — staggered 3D flip-in from scattered positions ──
        ...
        if (!isMobile && canvasRef.current) {
          cardRefs.current.forEach((el, i) => {
            ...
            // Each card rotates in on its own axis with a slight depth feel
            gsap.set(el, {
              opacity: 0,
              scale: 0.72,
              rotateY: i % 2 === 0 ? -22 : 22,
              rotateZ: pos.rotate, // keep final rotate baked in
              y: 40,
            ...
            gsap.to(el, {
              ...
              delay: 0.35 + i * 0.07,
              onComplete: () => {
                // Clear rotateY so hover CSS rotate works cleanly
                gsap.set(el, { clearProps: "rotateY,y,scale,opacity" });
              },
            });
          });
        }
  ...
  return (
    <section ref={sectionRef} className={styles["moodboard-wrapper"]}>

      <div className={styles["moodboard-header"]}>
        <h1 ref={h1Ref}>MOODBOARD</h1>
        <p ref={descRef}>
          Sharing personal thoughts, work-in-progress ideas, and deep-dives
          about design. Learnings from a decade in the industry.
        </p>
